...
def read_data(in_path):
    ...
    return [read_dataset(path_i) for path_i in paths]

def read_dataset(path_i):
    ...
    for line_i in file_i.readlines():
        ...
        raw_x=raw_x.replace("'",'')
        x_i=np.array(raw_x.split(' ')).astype(np.float64)
...
in_path="3DHOI_data/votes"
datasets=read_data(in_path)
print(len(datasets))